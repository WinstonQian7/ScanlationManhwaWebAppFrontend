...
export interface SearchBarProps {
  ...
  setUserInputCallback: (title: string) => void;
  ariaLabel?: string;
}
...
export function SearchBar({
  ...
  const searchInputListener = (e: React.FormEvent) => {
    if (e.target instanceof HTMLInputElement) {
      ...
      filteredSearchResults && filteredSearchResults.forEach(result => { 
        if (result.toLowerCase() === currentUserSearch.toLowerCase()) { 
          setUserInputCallback(currentUserSearch);
        }
      });
  ...
  const onClickSelectSearchOption = (e: React.MouseEvent) => {
    if (e.target instanceof HTMLDivElement) {
      ...
      setUserInputCallback(userSearch);
    }
    setDropdownOpen(false);
  };
  ...
  const filterSearch = ({searchOptions, userSearch, searchOptionDisplayLimit=5}: BaseSearchFilter) => {
    ...
    // console.log(`matched searches: ${matchedSearches} for input: ${userSearch}`, matchedSearches, userSearch);
